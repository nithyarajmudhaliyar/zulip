...
export function update_custom_profile_field_ui(
    user_id: number,
    field: {id: number; value: string | null | undefined},
): void {
    const field_id = field.id;
    const field_value = field.value;

    if (field_value === undefined || typeof field_value !== "string") {
        return;
    }
    ...
    if (user_id === people.my_current_user_id()) {
        ...
        if ($personal_input.length > 0) {
            $personal_input.val(field_value);
        }
    }

    const $manage_user_form = $("#edit-user-form");
    if ($manage_user_form.length > 0) {
        const modal_user_id = Number.parseInt($manage_user_form.attr("data-user-id") ?? "0", 10);
